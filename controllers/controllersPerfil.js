...
async function updateProfile(req, res) {
    try {
        const sessaoUsuario = req.session.usuario;
        const dadosDoFormulario = req.body;

        const dadosParaAtualizar = {};

        if (dadosDoFormulario.nome !== sessaoUsuario.nome) {
            dadosParaAtualizar.nome = dadosDoFormulario.nome;
        }
        if (dadosDoFormulario.e_mail !== sessaoUsuario.email) {
            dadosParaAtualizar.e_mail = dadosDoFormulario.e_mail;
        }

        if (dadosDoFormulario.telefone !== sessaoUsuario.telefone) {
        ...
        if (Object.keys(dadosParaAtualizar).length > 0) {
            ...
            await Usuario.update(dadosParaAtualizar, {
                where: { id_usuario: sessaoUsuario.id }
            });

            req.session.usuario = Object.assign(sessaoUsuario, {
                nome: dadosParaAtualizar.nome || sessaoUsuario.nome,
                email: dadosParaAtualizar.e_mail || sessaoUsuario.email,
                telefone: dadosParaAtualizar.telefone || sessaoUsuario.telefone,
            });
        } else {
