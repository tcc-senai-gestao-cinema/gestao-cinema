      ...
      diasUnicos.forEach((data, index) => {
        ...
        input.addEventListener('change', () => {
          ...
          horariosDoDia.forEach((programacao, hIndex) => {
            ...
            // Adicionar evento para mostrar botão comprar quando horário for selecionado
            inputH.addEventListener('change', () => {
              btnComprar.style.display = 'block';
              btnComprar.onclick = () => {
                const programacaoSelecionada = programacao.id_programacao;
                const localExibicao = programacao.id_local_de_exibicao;
                
                // Redirecionar para página de seleção de assentos
                window.location.href = `/distribuicao-do-publico?programacao_id=${programacaoSelecionada}&local_id=${localExibicao}&filme_id=${filmeId}`;
              };
            });
